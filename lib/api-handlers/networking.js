...
function normalizePerson(doc) {
    ...
    return {
        ...
        dateMet: parseDateKey(doc.dateMet),
        ...
        lastContacted: parseDateKey(doc.lastContacted || doc.dateMet),
        nextFollowUpAt,
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            const doc = {
                ...
                dateMet: body.dateMet ? String(body.dateMet).trim() : null,
                mutualConnections: String(body.mutualConnections || '').trim(),
                category: String(body.category || '').trim(),
                tags: Array.isArray(body.tags) ? body.tags.map((t) => String(t).trim()).filter(Boolean) : [],
                notes: String(body.notes || '').trim(),
                lastContacted: body.lastContacted ? String(body.lastContacted).trim() : null,
                ...
                lastInteractionAt: body.lastInteractionAt ? new Date(body.lastInteractionAt) : (body.lastContacted ? new Date(body.lastContacted) : null),
                createdAt: now,
                updatedAt: now,
            };
