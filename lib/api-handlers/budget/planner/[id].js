...
const COLLECTION = 'budget_plans';

async function getPlannerCollection(db) {
    const collections = (await db.listCollections().toArray()).map((c) => c.name);
    if (collections.includes('budget_plans')) {
        return db.collection('budget_plans');
    }
    if (collections.includes('budgetPlans')) {
        return db.collection('budgetPlans');
    }
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const collection = await getPlannerCollection(db);
