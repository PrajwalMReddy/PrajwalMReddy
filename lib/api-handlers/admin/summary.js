...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const [
            ...
            db.collection('networking_people').find({}).toArray().catch(() => []),
        ]);
        ...
        let networkingRecentlyAdded = 0;
        let networkingOverdueFollowUps = 0;
        let networkingUpcomingFollowUps = 0;
        const upcomingFollowUpList = [];

        (allPeople || []).forEach((person) => {
            if (person.createdAt && new Date(person.createdAt) >= sevenDaysAgo) {
                networkingRecentlyAdded += 1;
            }
            if (person.nextFollowUpAt && person.followUpStatus !== 'completed') {
                const isOverdue = person.nextFollowUpAt < todayStr;
                if (isOverdue) {
                    networkingOverdueFollowUps += 1;
                } else {
                    networkingUpcomingFollowUps += 1;
                }
                upcomingFollowUpList.push({
                    ...
                    isOverdue,
                    followUpNotes: person.followUpNotes || '',
                });
            }
        });

        upcomingFollowUpList.sort((a, b) => (a.nextFollowUpAt || '').localeCompare(b.nextFollowUpAt || ''));
        ...
        return res.status(200).json({
            ...
            networking: {
                ...
                recentlyAdded: networkingRecentlyAdded,
                overdueFollowUps: networkingOverdueFollowUps,
                upcomingFollowUps: networkingUpcomingFollowUps,
                totalFollowUpsDue: networkingOverdueFollowUps + networkingUpcomingFollowUps,
                recentFollowUps: upcomingFollowUpList.slice(0, 5),
            },
        });
    } catch (err) {
