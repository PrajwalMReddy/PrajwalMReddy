...
const createDueInstances = async (collection, source) => {
    ...
    const latestInstance = await collection
        .find({
            $or: [
                { _id: source._id },
    ...
    const isCurrentCompleted = Boolean(latestInstance?.completed);
    ...
    // Only create the next recurring task when the current task's deadline has passed (startDate < today),
    // or if the current task was already marked completed.
    if ((startDate < today || isCurrentCompleted) && (!endDate || nextDate <= endDate)) {
        currentOrder += 1;
        instances.push({
            title: source.title,
            completed: false,
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            const { title, completed, dueDate, priority, description, subtasks, estimatedTime, recurrence, recurrenceDays, recurrenceUntil, scheduledAt, tags } = req.body || {};
