...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'DELETE') {
            const existing = await interactionsColl.findOne({ _id });
            const result = await interactionsColl.deleteOne({ _id });
            if (result.deletedCount === 0) {
                return res.status(404).json({ error: 'Interaction not found' });
            }

            if (existing && existing.personId && ObjectId.isValid(existing.personId)) {
                const peopleColl = db.collection(PEOPLE_COLLECTION);
                const latestRemaining = await interactionsColl
                    .find({ personId: String(existing.personId) })
                    .sort({ date: -1, createdAt: -1 })
                    .limit(1)
                    .toArray();

                await peopleColl.updateOne(
                    { _id: new ObjectId(existing.personId) },
                    {
                        $set: {
                            lastInteractionAt: latestRemaining.length > 0 ? new Date(latestRemaining[0].date) : null,
                            updatedAt: new Date(),
                        },
                    }
                );
