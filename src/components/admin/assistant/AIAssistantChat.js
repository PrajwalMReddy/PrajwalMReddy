...
const AIAssistantChat = ({ externalPrompt = '', onClearPrompt }) => {
    const [messages, setMessages] = useState([
        {
            role: 'assistant',
            content: "Hello Prajwal. I'm your executive assistant connected directly to your workspace tools (tasks, calendar, budget, notes, and emails). How can I assist with your operations today?",
    ...
    // Handle external prompt passed from daily briefing cards
    useEffect(() => {
                    ...
                    setMessages((prev) =>
                        prev.map((msg) =>
                            msg.id === msgId
                                ? {
                                      ...msg,
                                      content: data.reply || msg.content || 'Request processed.',
                            ...
                            msg.id === msgId
                                ? {
                                      ...
                                      content: msg.content
                                          ...
                                          : `Error: ${err.message || 'Unable to connect to Claude.'}`,
                                      isError: true,
                                      isStreaming: false,
                                  }
                                : msg
            ...
            setMessages((prev) =>
                prev.map((msg) =>
                    msg.id === msgId
                        ? {
                              ...msg,
                              content: `Error: ${err.message || 'Unable to connect to Claude.'}`,
                              isError: true,
                              isStreaming: false,
                          }
                        : msg
    ...
    const handleClearHistory = () => {
        setMessages([
            {
                role: 'assistant',
                content: "Conversation history cleared. I'm ready with full fresh context.",
                timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            },
        ]);
        assistantApi.sendChatMessage('reset', 'admin_session', true).catch(() => {});
    };
        ...
        <div className="ai-box">
            ...
            <div className="ai-box-header">
                <div className="ai-box-title-group">
                    <div className="ai-box-icon" style={{ background: '#2563eb' }}>
                    ...
                    <div className="ai-box-titles">
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                            <h3 className="ai-box-title">Operations Assistant</h3>
                            <span style={{ fontSize: '0.68rem', fontWeight: 700, background: '#eff6ff', color: '#2563eb', padding: '0.1rem 0.4rem', borderRadius: '4px', border: '1px solid #dbeafe' }}>
                                Claude 3.5 Sonnet
                            </span>
                        </div>
                        <div className="ai-box-status">
                            <span className="ai-status-indicator status-online" />
                            <span className="ai-status-text">Global Full Context Active</span>
                        </div>
                    </div>
                </div>
                    ...
                    return (
                        <div key={index} className={`ai-msg ${isUser ? 'is-user' : 'is-assistant'}`}>
                            ...
                            <div className="ai-msg-bubble">
                                ...
                                {/* Tool execution and cache badges */}
                                {((Array.isArray(msg.toolCalls) && msg.toolCalls.length > 0) || msg.cacheStats) && (
                                    <div style={{ marginTop: '0.45rem', display: 'flex', gap: '0.3rem', flexWrap: 'wrap', alignItems: 'center' }}>
                                        {Array.isArray(msg.toolCalls) && msg.toolCalls.map((tool, tIdx) => (
                                            <span
                                                key={tIdx}
                                                style={{
                                                    fontSize: '0.68rem',
                                        ...
                                        {msg.cacheStats && msg.cacheStats.cacheReadTokens > 0 && (
                                            <span
                                                style={{
                                                    fontSize: '0.68rem',
                                                ...
                                                title={`${msg.cacheStats.cacheReadTokens} tokens read from prompt cache (90% discount)`}
                                            >
                                                ⚡ 90% Cached ({msg.cacheStats.cacheReadTokens} tokens)
                                            </span>
                                        )}
                                        {msg.cacheStats && msg.cacheStats.cacheCreatedTokens > 0 && (
                                            <span
                                                style={{
                                                    fontSize: '0.68rem',
                                                ...
                                                title="Prompt cached for 5 minutes"
                                            >
                                                ⚡ Cached Session
                                            </span>
                                        )}
                                ...
                                {/* Action Confirmation Cards (Requirement 5) */}
            ...
            <div className="ai-box-footer">
                <div className="ai-box-input-wrap">
                    <textarea
                        ...
                        placeholder="Ask Claude anything about your briefing, tasks, or budget..."
                        rows={1}
                        value={input}
