export const home = {
    en: {
        // Home Page
        plane: "------\n" + "| | # \\                                      |\n" + "| ____ \\_________|----^\"|\"\"\"\"\"|\"\\___________ |\n" + " \\___\\   PR - 17 >>    `\"\"\"\"\"\"\"\"     =====  \"|D\n" + "       ^^-------____--\"\"\"\"\"\"\"\"\"\"+\"\"--_  __--\"|\n" + "                   `\"\"|\"-->####)+---|`\"\"     |\n" + "                                 \\  \\\n" + "                                <- O -)\n" + "                                  `\"' \n",

        homeTitle: "Hello World!",
        homeIntro: "I'm [ˈprə.dʒʋəl ˈrɛ.ɖːi]",
        homeSubtitle: "a student, programmer, and builder.",
        homeDescription: "I am a college student at Cornell University, studying a double major in Linguistics and Computer Science, with a specialization in Economics. I am also passionate Urban Planning, often developing applications that focus on issues in these areas. My other non-academic interests include world building and conlanging.",

        // Skills Section
        skillsTitle: "My Programming Skills",
        skills: {
            python: "Python",
            javaKotlin: "Java and Kotlin",
            cpp: "C and C++",
            dartFlutter: "Dart + Flutter",
            php: "PHP",
            sql: "SQL + MySQL",
            web: "HTML + CSS + JavaScript",
            rust: "Rust",
            arduino: "Arduino"
        },

        // Featured Projects Section
        featuredProjectsTitle: "Featured Projects",
        featuredProjectsSubTitle: "See All Projects >",
    }, kn: {
        // Home Page
        plane: "------\n" + "| | # \\                                      |\n" + "| ____ \\_________|----^\"|\"\"\"\"\"|\"\\___________ |\n" + " \\___\\   ಪ್ರರೆ - ೧೭ >>   `\"\"\"\"\"\"\"\"     =====  \"|D\n" + "       ^^-------____--\"\"\"\"\"\"\"\"\"\"+\"\"--_  __--\"|\n" + "                   `\"\"|\"-->####)+---|`\"\"     |\n" + "                                 \\  \\\n" + "                                <- O -)\n" + "                                  `\"' \n",

        homeTitle: "ನಮಸ್ಕಾರ!",
        homeIntro: "ನಾನು ಪ್ರಜ್ವಲ ರೆಡ್ಡಿ",
        homeSubtitle: "ಕಲಿಗ, ಹಮ್ಮುಗ, ಮತ್ತು ಕಟ್ಟುಗ.",
        homeDescription: "ನಾನು ಕೊರ್ನೆಲ ಮೇಲ್ಕಲಿಮನೆಯಲ್ಲಿ ಕಲಿಗರಾಗಿದ್ದೇನೆ. ನುಡಿಯರಿಮೆ, ಎಣಿಯರಿಮೆ, ಮತ್ತು ಸೊಮ್ಮರಿಮೆಯನ್ನು ಓದುತ್ತಿದ್ದೇನೆ. ಅಶ್ಟೇ ಅಲ್ಲ, ನಗರ ಹಮ್ಮುಗೆಯ ಮೇಲೆ ತುಂಬಾ ಇಶ್ಟಪಡುತೇನೆ. ನನ್ನ ಇತರ ಆಸಕ್ತಿಗಳಲ್ಲಿ ಪ್ರಪಂಚ ಕಟ್ಟುವಿಕೆ ಮತ್ತು ನುಡಿ ಕಟ್ಟುವಿಕೆ ಎರಡು.",

        // Skills Section
        skillsTitle: "ನನ್ನ ಹಮ್ಮುಗಾರಿಕೆಯ ಕಲಿಕೆಗಳು",
        skills: {
            python: "ಪಾಯ್ತನ್",
            javaKotlin: "ಜಾವ ಮತ್ತು ಕಾಟ್ಲಿನ್",
            cpp: "ಸಿ ಮತ್ತು ಸಿ++",
            dartFlutter: "ಡಾರ್ಟ್ + ಪ್ಲಟರ್",
            php: "ಪಿ.ಏಚ್.ಪಿ.",
            sql: "ಸೀಕ್ವಲ್ + ಮಯ್ಸೀಕ್ವಲ್",
            web: "ಏಚ್.ಟಿ.ಎಮ್.ಎಲ್. + ಸಿ.ಎಸ್.ಎಸ್. + ಜಾವಸ್ಕ್ರಿಪ್ಟ್",
            rust: "ರಸ್ಟ್",
            arduino: "ಆರ್ಡ್ವೀನೊ"
        },

        // Featured Projects Section
        featuredProjectsTitle: "ಬೆಳಗಿಸಿದ ಯೋಜನೆಗಳು",
        featuredProjectsSubTitle: "ಯೋಜನೆಗಳೆಲ್ಲ ನೋಡಲು >",
    }
};
